...
class TestNew:
    def test_new_with_default_options(self, tmp_path, capsys, linter):
        execli(f'new {tmp_path}')
        linter.run([str(tmp_path)])
    ...
    def test_new_bare_project(self, tmp_path, opts, linter):
    ...
    @pytest.mark.parametrize('opts', ['--bare'])
    def test_with_dot_target_should_be_convert_to_proj(self, tmp_path, opts, linter):
        cwd = os.getcwd()
    ...
    def test_new_with_options_app(self, tmp_path, opts, linter):
        execli(f'new {tmp_path} {opts}')
        linter.run([str(tmp_path)])
        fset = {i.name for i in tmp_path.glob('*')}
    ...
    def test_new_with_options_proj(self, tmp_path, opts, linter):
        execli(f'new {tmp_path} {opts}')
        linter.run([str(tmp_path)])
        fset = {i.name for i in tmp_path.glob('*')}
    ...
    def test_new_with_options_pypi(self, tmp_path, opts, linter):
        execli(f'new {tmp_path} {opts}')
        linter.run([str(tmp_path)])
        fset = {i.name for i in tmp_path.glob('*')}
    ...
    def test_new_with_options_app_proj_pypi(self, tmp_path, opts, linter):
        execli(f'new {tmp_path} {opts}')
        linter.run([str(tmp_path)])
        fset = {i.name for i in tmp_path.glob('*')}
    ...
    def test_new_app_should_be_underscore(self, tmp_path, opts, linter):
        execli(f'new {tmp_path} {opts}')
        linter.run([str(tmp_path)])
        fset = {i.name for i in tmp_path.glob('*')}
    ...
    def test_new_with_proj_should_be_camel(self, tmp_path, opts, linter):
        execli(f'new {tmp_path} {opts}')
        linter.run([str(tmp_path)])
        fset = {i.name for i in tmp_path.glob('*')}
...
class TestRepo:
    ...
    def test_from_local(self, tmp_path, opts, linter):
    ...
    def test_with_json_config(self, tmp_path, opts, linter):
    ...
    def test_with_file_config(self, tmp_path, linter):
    ...
    @pytest.mark.parametrize('opts', ['--json={"app":"foobar","bare":true}'])
    def test_json_config_should_override_file_config(self, tmp_path, opts, linter):
        file_opt = Path(__file__).parent.joinpath('fixture.json')
